...
\subsection{General framework}\label{sec:cosmo:quintessence:framework}
    ...
    We have $\widetilde{\rho}_s\propto (Aa)^{-3(1+w_s)} \equiv A^{-3(1+w_s)}\rho_s$, where $\rho_s=a^{-3(1+w_s)}$ defines a scaled energy density, conserved in the Einstein frame, and $a$ the Einstein-frame scale factor. Now
    \begin{equation}
    ...
    \begin{figure}[h]
        ...
        \caption{The evolution of the (a)symmetron effective potential in (dashed red) solid blue. \comment{Describe time.} \iftime{Add two balls.}
    ...
    \end{equation}
    We consider the matter content to be dust-like, so that $\widetilde{T}\ped{m}\simeq -\widetilde{\rho}$. The effective potential is then given by~\citep{hinterbichlerSymmetronCosmology2011}\lcomment{Mention $V\ped{eff}=V(\phi)+A(\phi)\rho$?}
