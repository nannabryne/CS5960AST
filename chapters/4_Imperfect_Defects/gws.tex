...
Static, planar domain walls do not themselves produce gravitational radiation. Introducing asymmetry to the system, as we do when adding perturbations, can give rise to non-vanishing stress--energy tensor components in the TT gauge. %
...
% \iftime{Comment: Let us mention that introducing $v\neq 0$, may be accounted for by $\sigma$ as the limits}
% Let us mention that introducing $v\neq 0$, may be accounted for by $\sigma$ as the limis \blahblah (appendix???)
...
\subsection{Fourier-space stress--energy tensor}\label{sec:pertwalls:gws:Fourier_SE_tensor}
    ...
    Let us assume $\epsilon_{,x}=0$ such that~$\epsilon(\tau,x,y)=\varepsilon(\tau)\sppt(y)$. % with eigenvalue $p^2$. %
