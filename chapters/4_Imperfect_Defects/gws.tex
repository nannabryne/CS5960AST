...
\subsection{\tmptitle{Dynamics of gravitational waves in expanding universe}}
    % \citep{kawasakiStudyGravitationalRadiation2011}
    From~\cref{sec:GR:lingrav:gws_FLRW} we have an expression for $h\_{ij}(\tau, \vec{k})$ in a universe with $a\propto \tau^\alpha$. Consider $\alpha\in \Integer$. Assuming homogeneous initial conditions at $\tau\ped{init}=\tau_\ast$, the tensor perturbations are given by~\cref{eq:GR:lingrav:hij_expression_of_Greens_and_source} and~\cref{eq:GR:lingrav:Greens_function_n} with $n=\alpha-1$. For convenience, we use a linear polarisation basis (see~\cref{sec:GR:lingrav:polbasis}) and the decomposition $\ah_P=H^{1}_P + H^2_P $ such that
    % which we will write as $\ah$
    \begin{equation}
        \begin{split}
            H_P^1(\tau, \vec{k}) &= +\RiccatiBessel[n][1](k\tau) \integ{\eta}[\tau_\ast][\tau]  \RiccatiBessel[n][2](k\eta)  \Src_P(\eta, \vec{k}), \\
            H_P^2(\tau, \vec{k}) &= -\RiccatiBessel[n][2](k\tau) \integ{\eta}[\tau_\ast][\tau]  \RiccatiBessel[n][1](k\eta) \Src_P(\eta, \vec{k}).
        \end{split}
    \end{equation}
    $\Src_P= 16\ppi G\nped{N} a T\ap{TT}_P/k^2$ contains the TT-projected stress--energy tensor. The conformal time derivative $\dot{\ah} = \dot{H^1_P} + \dot{H^2_P} $ is given by
    \begin{equation}
        \begin{split}
            \dot{H}_P^1(\tau, \vec{k})  &= +k \,\bclosed{ \RiccatiBessel[\alpha][1](k\tau) -  n\sphBessel[n][1](k\tau) } \integ{\eta}[\tau_\ast][\tau]  \RiccatiBessel[n][2](k\eta)  \Src_P(\eta, \vec{k}),  \\
            \dot{H}_P^2(\tau, \vec{k})  &= -k \,\bclosed{ \RiccatiBessel[\alpha][2](k\tau) -  n\sphBessel[n][2](k\tau) } \integ{\eta}[\tau_\ast][\tau]  \RiccatiBessel[n][1](k\eta)  \Src_P(\eta, \vec{k}).  
        \end{split}
    \end{equation}
    ...
    % \begin{equation}
    %     \begin{split}
    %         \dot{H}_P^1(\tau, \vec{k})  &= +k \,\bclosed{ \RiccatiBessel[\alpha][1](k\tau) -  n\sphBessel[n][1](k\tau) } \integ{\eta}[\tau\ped{init}][\tau]  \RiccatiBessel[n][2](k\eta)  \Src_P(\eta, \vec{k})  \\
    %         \dot{H}_P^2(\tau, \vec{k})  &= -k \,\bclosed{ \RiccatiBessel[\alpha][2](k\tau) -  n\sphBessel[n][2](k\tau) } \integ{\eta}[\tau\ped{init}][\tau]  \RiccatiBessel[n][1](k\eta)  \Src_P(\eta, \vec{k})  
    %     \end{split}
    % \end{equation}
...
\subsection{Fourier space stress--energy tensor}\label{sec:pertwalls:gws:Fourier_SE_tensor}
    \begin{bullets}
        ...
        \item TT gauge
    \end{bullets}
    From~\cref{sec:pertwalls:thinwall} we found that the SE tensor of a thin domain wall in an expanding universe looks like~\cref{eq:pertwalls:thinwall:SE_tensor}, which for the case of a wall in the $xy$-plane reduces to
    % \begin{equation}
    ...
    \begin{equation}
        \begin{split}
            ...
            &= -a (\tau)\sigma(\tau) \varPhi_l(z-z\ped{w} ) \,\epsilon\_{,i}, %\partial\_{i} \epsilon
        \end{split}
    \end{equation}
    where $z\ped{w} = z_0 + \epsilon(x\^a)$ and $\varPhi_l$ is a Gaussian with standard deviation $l=\delta\ped{w}/2$. We go further and look at this quantity in Fourier space:
    \begin{equation}
        \begin{split}
            % T\_{ab}(\tau, \vec{k})  = 
            T\ap{w}_{ab}(\tau, \vec{k}) &=  -a (\tau)\sigma(\tau)  \, \eta\_{ab} \, \mathscr{D}\ped{w}(k_z) \integ[2]{x} \eu[-\im k_z \epsilon] \eu[\im (k_x x+k_y y)], \\
            % T\_{(iz)}(\tau, \vec{k})  
            T\ap{w}_{iz}(\tau, \vec{k})
            &= -a (\tau)\sigma(\tau) \,\mathscr{D}\ped{w}(k_z)  \integ[2]{x} \epsilon\_{,i}  \eu[-\im k_z \epsilon] \eu[\im (k_x x+k_y y)],%\partial\_{i} \epsilon
        \end{split}
    \end{equation}
    where $\mathscr{D}\ped{w}(k_z) = \exp{ -\im k_z z_0-{(k_z\delta\ped{w})}^2/8 }$ adjusts for wall width and origin. %
    % \begin{equation}
    %     \begin{split}
    %         % T\_{ab}(\tau, \vec{k})  = 
    ...
    % \end{equation}
    \begin{equation}
        \underbrace{\integ{y} \eu[-\im k_z \varepsilon(\tau) \cdot \sppt(y)] \eu[\im k_y y]}_{I\ped{s}}  \quad \text{and}\quad %
        \underbrace{\varepsilon(\tau)\integ{y} \partial_y \sppt\, \eu[-\im k_z \varepsilon(\tau) \cdot \sppt(y)] \eu[\im k_y y]}_{I\ped{a}}
    \end{equation}
    are all we need to solve to have a completely analytic expression for $T\ap{w}_{ij}(\tau, \vec{k})$. %
    % \begin{equation}
...
\subsection{Examples}
        ...
        \end{figure}
        % ------------------------------
        Both the perturbation scale parameter $u$ \emph{and} the initial amplitude $\varepsilon_\ast$ contribute to the GW signature. 
