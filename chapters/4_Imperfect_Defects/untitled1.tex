...
We will use $s\equiv \tau/\tau_\ast$ as our time variable, and $\omega=p\tau_\ast$ as the eigenvalue, when it comes to that. In a universe with $a = a_\ast s^\alpha$ we have $\upsilon = s^{-3\alpha}$ and thus  %the surface tension \cringe{is} $\sim {(1-s^{-3\alpha})}^{3/2}$, thus
\begin{equation}
    \frac{\sigma '}{\sigma} 
...
\subsection{Solution in matter-dominated universe}
    ...
    \begin{equation}\label[eq]{eq:pertwalls:untitled1:eps_s_II_MD_w}
        e^{\text{\eqregimenum{II}}}(s) \simeq   w(s) \cdot \exp{ - \integ{t}[][s] d(t) }.%; \quad w(s) = \frac{c\sphBessel[2](qs) + d \sphNeumann[2](qs)}{s^2}.
    \end{equation}
    ...
    \begin{equation}\label{eq:pertwalls:untitled1:eps_s_complete_MD}
        ...
        \end{cases}
    \end{equation}
    In~\cref{fig:pertwalls:untitled1:demo_analytical_epsilon} we demonstrate how this solution looks like for arbitrary $\omega$, in the different steps described above. \comment{Should include solution in the case where surface tension is constant.}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%            FIGURE            %%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \begin{figure}[H]
        \centering
        \includegraphics[width=\linewidth]{Background/demo_analytical_epsilon.pdf}
        %%%%%%%%%%
...
\subsection{\tmptitle{Review}}
    ...
    \begin{figure}[h]
        ...
        %\label{fig:pertwalls:untitled1:932434t}
    \end{figure}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \paragraph{Scaling.} %
    It turns out that if plotted over the time variable $t_\omega \equiv \omega(s-1)=p(\tau-\tau_\ast)$, the solution in~\cref{eq:pertwalls:untitled1:eps_s_complete_MD} is \checkthis{equal up to normalisation in $\varepsilon_\ast$}
...
% Assume that such a formation induces a perturbation to the wall that moves the wall normal coordinate from $z_0$ to $z_0 + \varepsilon_\ast$. This gives the initial condition $\epsilon(1)=\varepsilon_\ast$ to the equation of motion for $\epsilon(s)$ with $\alpha=2$, namely
...
% \begin{equation}
%     \epsilon(s) = \varepsilon_\ast \cdot \bclosed{ 1 + c_1 (s-1) + c_2 {(s-1)}^2 + c_3 {(s-1)}^3 + \dots }.
% \end{equation}
% % We immediately see that the initial condition 
...
%     \epsilon(s) = \varepsilon_\ast \cdot \bclosed{ 1 - \frac{q^2}{5} {(s-1)}^2 + \frac{q^2}{35} {(s-1)}^3 } + \BigO{\pclosed{s-1}^4}, \quad s \gtrsim 1
% \end{equation}
...
%     \epsilon(s) \simeq \varepsilon_\ast \cdot  w(s) \cdot \exp{ - \integ{t}[][s] \gamma(t) }.%; \quad w(s) = \frac{c\sphBessel[2](qs) + d \sphNeumann[2](qs)}{s^2}.
% \end{equation}
% Employing this ansatz in the eom gives
% \begin{equation}
...
% \begin{equation}
%     \epsilon(s) \simeq \varepsilon_\ast  \cdot \frac{ A \Bessel[-5/2](qs) + B \Neumann[-5/2](qs) }{s^{5/2}} \frac{s^{9/2}}{{(s^6-1)}^{3/4}}, \quad s \geq s\ped{sow}
% \end{equation}
...
% \begin{bullets}
%     \item We assume given values of $\varepsilon_\ast$ and $q$
...
%     \item Two free parameters: As we can see, changing the amplitude $\varepsilon_\ast$ does not change the motion, but varying the wavenumber $q$ does
% \end{bullets}


% \begin{equation}
%     \epsilon_q(s, x, y) = \varepsilon_\ast \cdot \begin{cases}
