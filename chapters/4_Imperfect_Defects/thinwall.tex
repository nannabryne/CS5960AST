...
According to~\citet{vilenkinCosmicStringsOther1994} \speak{(This was~\citeyear{vilenkinCosmicStringsOther1994}, so maybe outdated?)}, no known gauge choice can reduce~\cref{eq:pertwalls:thinwall:eom_domain_wall_general} to an exactly solvable linear equation. %
...
We perform the variation in~\cref{eq:pertwalls:eom_wall:SE_tensor_NG} for the scenario in question to find the SE tensor associated with the wall motion. The detailed calculation is found in~\cref{app:walls:SE_tensor}\rcomment{Maybe not...}. The non-vanishing spatial components are
\begin{equation}\label{eq:pertwalls:thinwall:SE_tensor}
    \begin{split}
        % T\_{ab}(\tau, \vec{x})
        \Tw\_{ab}&=  -a \sigma\ped{w} \varPhi_l(z-z\ped{w}) \cdot \eta\_{ab} ,\\
        % T\_{(i3)}(\tau, \vec{x})
        \Tw\_{(i3)}&= -a \sigma\ped{w}\varPhi_l(z-z\ped{w} ) \cdot\epsilon\_{,i}, %\partial\_{i} \epsilon
    \end{split}
\end{equation}
