...
\end{equation}
We simplify our equations tremendously if we keep $n\^\mu$ unaffected by the variation, and solve the one-dimensional problem with
\begin{equation}
    x_\bot \to \pert{x}_\bot = x_\bot + \epsilon.
\end{equation}
% only we keep $n\^\mu$ unaffected by the variation. 
The resulting equation of motion for the scalar perturbation reads
\begin{equation}
...
% \subsection{Stress--energy tensor}\label{sec:pertwalls:thinwall:SE_tensor}
    We perform the variation in~\cref{eq:pertwalls:eom_wall:SE_tensor_NG} for the scenario in question to find the SE tensor associated with the wall motion. The detailed calculation can be found in~\cref{app:derivations:SE_NG}. The non-vanishing spatial components are
    \begin{equation}\label{eq:pertwalls:thinwall:SE_tensor}
        \begin{split}
            T\_{ab}(\tau, \vec{x})  &=  -a \sigma \, \varPhi_l(z-z\ped{w}) \, \eta\_{ab} \\
            T\_{(i3)}(\tau, \vec{x})  &= -a \sigma \,\varPhi_l(z-z\ped{w} ) \,\epsilon\_{,i} %\partial\_{i} \epsilon
        \end{split}
    \end{equation}
...
\subsection{Time-dependent surface tension}
    ...
    we need to put this inside of the integral in the Nambu--Goto action in~\cref{eq:pertwalls:eom_wall:total_action}. We immediately see that this is equivalent to letting $a^3\to a^3 \sigma$, which amounts to 
    ...
    \end{equation}
    This extra term will introduce the model-dependence. As we show in \nc{some section}[next?], the surface tension in the thin-wall approximation is 
