...
\subsection{The stationary \( \Zn \) kink}\label{sec:cosmo:defects:ex_Z2_kink}
    ...
    \end{equation}
    where $\abs{N-M}\leq 1$ and $x_i < \widebar{x}_j < x_{i+1}$. \comment{rewrite} This describes the allowed system of $N$ kinks and $M$ antikinks aligned in an alternating structure.
...
\subsection{Defect formation}\label{sec:cosmo:defects:formation}
    ...
    To study phase transitions, it is helpful to use the \newconcept{effective potential} that takes into account the interaction between some field and a background~\citep{vachaspatiKinksDomainWalls2006,kolbEarlyUniverse1990}. We will demonstrate this in~\cref{sec:cosmo:quintessence} for a particular scalar-field theory. %
    % \lcomment{Rewrite this.}
    % \rephrase{An effective potential can ensure that a scalar field does not really show itself until some critical temperature. % $T\ped{cr}$. 
