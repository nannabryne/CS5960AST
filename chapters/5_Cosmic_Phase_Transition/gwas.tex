...
\subsection{Code output and interpretation}\label{sec:PT:gwas:data_comparison}
    ...
    % \end{equation}
    specifically the \textit{Scipy} library's \texttt{cumptrapz} method in~\texttt{Python}. We let $\hpA$ and $\hpB$ signify the solution to~\crefrange{eq:PT:gwas:mywaves_complete_formula}{eq:PT:gwas:mywaves_pi_plus_series} with $\epsA$ and $\epsB$ as input, respectively.
