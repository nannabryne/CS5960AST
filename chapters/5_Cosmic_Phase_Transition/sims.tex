...
\subsubsection{Nature of wall perturbation}
    ...
    We would like $\epsast \ll L_\#/2$, but simply by looking at~\cref{fig:pertwalls:mywalls:demo_analytical_epsilon} we can assume that this would require a very high spatial resolution. As a result, we choose to exaggerate the initial perturbation amplitude and risk higher-order effects.%\verb|..|
...
\subsection{Catalogue}
    ...
    Every simulation, labelled \simnum{0},~\simnum{1},\dots,~\simnum{7}, used a 4th order Runge-Kutta solver. 
    ...
    An additional, completely homogeneous simulation (no walls present) is run with the same background as simulation~\simnum{1}, except that the symmetron initial conditions are optimised as described~\cref{app:untitled2} with $\redshift\ped{i}=2.03$. %In particular, 
