...
% \footnote{\comment{A truth with modifications}; see discussion in~\cref{app:walls:surface_tension:new_limits}.} 
...
asymptotic wall width $\delta_\infty$ (\simnum{6}),\footnote{Which is to say we vary symmetron mass scale $\mu$, or Compton wavelength $L\nped{C}$.} %
initial perturbation amplitude $\epsast/L_\#$ (\simnum{0},~\simnum{2},~\simnum{5}) and 
perturbation scale $p/k_\#$ (\simnum{0},~\simnum{5}).\footnote{Scale $p$ is irrelevant in simulation~\simnum{0} as $\epsast=0$.} In the following, results from simulation~\simnum{6} are discarded (see~\cref{app:misc:sim6} for details). We also exclude simulation~\simnum{0}, as the comparison is trivial.
...
$(16\pm 5)\%$. Between simulations~\simnum{1},~\simnum{3}--\simnum{5} and~\simnum{7}, which all obey $\epsast< \wallsep/5=0.1L_\#$, the corresponding measures read $\max{(\Delta\varepsilon)} \in [13\pm 2]\epsast$ and $\avg*{\Delta \varepsilon}\in [7\pm 1]\epsast$, respectively. 
