...
\rephrase{In physics, the answer to a question is highly dependent on \emph{how the question was asked}.
...
\subsection{Energy--momentum tensor}\label{sec:GR:einstein:SE_tensor}
    ...
    \subsubsection{Conformal transformations}
        ...
        \rephrase{We let ${}\ap{(m)}T\triangleq T$ and ${}\ap{(m)}\wt{T}\tmpequiv \wt{T}$ denote the matter energy and momentum in the two frames for notational ease.} %
...
\begin{equation}\label{eq:GR:einstein:linearised_Einstein_eq}
    \pert{\mathcal{G}}\_{\mu\nu} = 8\ppi G\nped{N} \pert{T}\_{\mu\nu},
\end{equation}
where $\pert{\mathcal{G}}\_{\mu\nu}$ and $\pert{T}\_{\mu\nu}$ are the $o$th order perturbed Einstein and SE tensor, respectively. \Cref{eq:GR:einstein:linearised_Einstein_eq} is then solved order by order. 

For first-order perturbations to the metric, we get~\citep{jokelaGravitationalWaveMemory2022}
\begin{subequations}
    \begin{align}
