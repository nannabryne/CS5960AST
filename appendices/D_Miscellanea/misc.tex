...
\section{Simulation~\simnum{6}}\label{app:misc:sim6}
    ...
    % and the wall width $\delta\ped{w}\approx 7.3~\Mpch$, both 
    way smaller than the spatial resolution $\Delta_\# \simeq 1.3~\Mpch$. %  
    % ------------------------------
    % ----------- FIGURE -----------
    \begin{figure}[hb]
        \centering
        \includegraphics[width=\linewidth]{Appendices/sim6_err.png}
...
\section{On the continuous/discrete Fourier-comparison}\label{app:misc:CFT_DFT}
    ...
    \end{align}
    for $\lcoordk = (0,\lcoord{v},\lcoord{w})$. We leave this here. %This is one possible sanity test that would be performed if we had time.
    % \comment{Either find this result ($\sim$ 15 min), or remove this last part.}
